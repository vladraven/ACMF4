...
def run_test_21(params: ModelParameters) -> TestResult:
    """TEST 21 — Независимость численных результатов: Euler-Maruyama vs Milstein."""
    domain = SolverDomain(sid_buf=params.SID_buf, sid_max=params.SID_max, f_max=params.F_max)
    forcing = ForcingProfile().evaluate(0.0)

    engine_em = ACMFEngine(domain=domain, scheme="euler_maruyama")
    engine_mil = ACMFEngine(domain=domain, scheme="milstein")
    ...
    max_diff = float(np.max(np.abs(traj_em.states - traj_mil.states)))
    # Разница между EM и Milstein порядка O(sqrt(dt)) для стохастики
    is_consistent = max_diff < 0.5

    return TestResult(
        test_id="TEST_21",
        name="Solver Independence Convergence",
        status="PASSED" if is_consistent else "FAILED",
        details={"max_trajectory_discrepancy": max_diff},
    )