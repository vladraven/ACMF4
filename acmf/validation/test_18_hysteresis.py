...
def run_test_18(params: ModelParameters) -> TestResult:
    ...
    base_state = np.zeros(state_dim, dtype=np.float64)
    base_state[0:params.N_sub] = 2.0
    base_state[params.N_sub : params.N_sub + 4] = 0.5
    base_state[params.N_sub + 5] = 0.0

    scar_state = base_state.copy()
    scar_state[params.N_sub + 5] = 0.7
    ...
    traj_base = engine.simulate(
        initial_state=base_state, t_span=(0.0, 20.0), dt=0.05,
        drift_fn=drift_fn, diffusion_fn=diff_fn,
    )
    traj_scar = engine.simulate(
        initial_state=scar_state, t_span=(0.0, 20.0), dt=0.05,
        drift_fn=drift_fn, diffusion_fn=diff_fn,
    )

    final_distance = float(np.linalg.norm(traj_base.states[-1] - traj_scar.states[-1]))
    scar_difference = float(abs(traj_base.states[-1, params.N_sub + 5] - traj_scar.states[-1, params.N_sub + 5]))

    status = "PASSED" if final_distance > 0.0 and scar_difference > 0.0 else "FAILED"
